...
class QuestionComponent extends Container {
	...
	constructor(question: Question, tui: TUI, theme: Theme, onDone: (answer: string | null) => void, borderless = false) {
		...
		// Compact layout: top divider only (no full box), merged header + question
		// on one line, options list, help line. Keeps the widget short enough to
		// fit in an overlay without pushing other content around.
		if (!borderless) {
...
`;
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			// Render as an overlay docked at the bottom-left of the terminal, full
			// width, capped height. This sits where the editor normally is, avoids
			// stacking on top of the existing chat history, and sidesteps the
			// differential renderer's full-redraw path when total inline content
			// exceeds terminal height (which would otherwise cause scroll storms).
